...
use crate::util::{hi_dp_bc, refined_reciprocal};
...
impl Mul<f32> for Direction {
    type Output = Direction;
    fn mul(self, s: f32) -> Direction {
        Direction { p3: self.p3 * s }
    }
}
...
impl Div<f32> for Direction {
    type Output = Direction;
    fn div(self, s: f32) -> Direction {
        Direction {
            p3: self.p3 * refined_reciprocal(s),
        }
    }
}
...
impl DivAssign<f32> for Direction {
    fn div_assign(&mut self, s: f32) {
        self.p3 = self.p3 * refined_reciprocal(s)
    }
}
