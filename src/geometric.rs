use crate::util::{
    add_ss, b2b3a2a3, dp, flip_signs, rcp_nr1, shuffle_wwww, shuffle_wwyy, shuffle_wyzx,
    shuffle_wzxy, shuffle_xwww, shuffle_xxyz, shuffle_xyzx, shuffle_xzxy, shuffle_ywww,
    shuffle_yxyz, shuffle_yyzx, shuffle_yzxy, shuffle_zwww, shuffle_zyzx, shuffle_zzxy,
};
...
pub fn gprt<const F: bool>(a: f32x4, b: f32x4) -> f32x4 {
    ...
    let tmp = if F {
        shuffle_zzxy(a) * shuffle_zyzx(b)
    } else {
        shuffle_zyzx(a) * shuffle_zzxy(b)
    };
