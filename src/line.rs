...
impl Line {
    ...
    pub fn new(a: f32, b: f32, c: f32, d: f32, e: f32, f: f32) -> Line {
        Line {
            p1: f32x4::from_array([0.0, d, e, f]),
            p2: f32x4::from_array([0.0, a, b, c]),
        }
    }

    pub fn from_branch(b: Branch) -> Line {
        Line {
            p1: b.p1,
            p2: f32x4::splat(0.0),
        }
    ...
    pub fn from_ideal_line(l: IdealLine) -> Line {
        Line {
            p1: f32x4::splat(0.0),
            p2: l.p2,
        }
    ...
    pub fn project_point(self, a: Point) -> Line {
        (self | a) | a
    }

    // Project a line onto a plane
    pub fn project_plane(self, p: Plane) -> Line {
        (self | p) ^ p
    }
}
...
impl BitXor<IdealLine> for Line {
    type Output = Dual;
    fn bitxor(self, b: IdealLine) -> Dual {
        Branch { p1: self.p1 } ^ b
    }
}
...
impl BitOr<Line> for Line {
    type Output = f32;
    fn bitor(self, other: Line) -> f32 {
        dot11(self.p1, other.p1)[0]
    }
}

impl BitOr<Plane> for Line {
    type Output = Plane;
    fn bitor(self, p: Plane) -> Plane {
        Plane {
            p0: dotpl::<true>(p.p0, self.p1, self.p2),
        }
    }
}
...
impl Branch {
    ...
    pub fn normalized(self) -> Branch {
        Branch {
            p1: self.p1 * rsqrt_nr1(hi_dp_bc(self.p1, self.p1)),
        }
    }
...
impl Not for Branch {
    type Output = IdealLine;
    fn not(self) -> IdealLine {
        IdealLine { p2: self.p1 }
    }
}
