use crate::geometric::{gp11, gp12, gpmm, gprt};
use crate::sandwich::{sw012, sw312, swmm, swo12};
use crate::sqrt::rsqrt_nr1;
use crate::util::{bits_wwww, dp_bc, f32x4_abs, flip_signs, log, rcp_nr1};
use crate::{Line, Origin, Plane, Point, Rotor, Translator};
use core_simd::{f32x4, mask32x4};
use std::convert::From;
use std::ops::{Add, AddAssign, Div, DivAssign, Fn, Mul, MulAssign, Neg, Sub, SubAssign};
...
impl Motor {
    ...
    pub fn new(a: f32, b: f32, c: f32, d: f32, e: f32, f: f32, g: f32, h: f32) -> Motor {
        Motor {
            p1: f32x4::from_array([a, b, c, d]),
            p2: f32x4::from_array([h, e, f, g]),
        }
    }
    ...
    pub fn inverse(&self) -> Motor {
        ...
        let s = rsqrt_nr1(b2);
        let bc = dp_bc(
            flip_signs(self.p1, mask32x4::from_array([true, false, false, false])),
            self.p2,
        );
        ...
        let st = s * t * self.p1;
        let mut p2 = self.p2 * b2_inv
            - (flip_signs(st * st, mask32x4::from_array([true, false, false, false])));
    ...
    pub fn constrained(&self) -> Motor {
        let mask =
            bits_wwww(self.p1.to_bits() & f32x4::from_array([-0.0, 0.0, 0.0, 0.0]).to_bits());
    ...
    pub fn sqrt(self) -> Motor {
        ...
        .normalize() // TODO avoid extra copy of Motor
    }
    ...
    pub fn as_mat3x4(&self) {
        todo!();
    }
    pub fn as_mat4x4(&self) {
        todo!();
    }
}
...
impl Fn<(Plane,)> for Motor {
    extern "rust-call" fn call(&self, args: (Plane,)) -> Plane {
        Plane {
            p0: sw012::<false, true>(args.0.p0, self.p1),
        }
    }
}
