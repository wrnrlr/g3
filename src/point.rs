...
use crate::inner::{dot33, dotptl};
use crate::util::{flip_signs, rcp_nr1, shuffle_wwww};
use crate::{Branch, Dual, IdealLine, Line, Motor, Plane, Translator};
...
impl Point {
    ...
    pub fn reverse(&self) -> Point {
        Point {
            p3: flip_signs(self.p3, mask32x4::from_array([false, true, true, true])),
        }
    }

    // Project a point onto a line
    pub fn project_line(self, l: Line) -> Point {
        (self | l) ^ l
    }

    // Project a point onto a plane
    pub fn project_plane(self, p: Plane) -> Point {
        (self | p) ^ p
    }
}
...
impl BitAnd<Point> for Point {
    type Output = Line;
    fn bitand(self, other: Point) -> Line {
        !(!self ^ !other)
    }
}
impl BitAnd<Line> for Point {
    type Output = Plane;
    fn bitand(self, l: Line) -> Plane {
        !(!self ^ !l)
    }
}
impl BitAnd<IdealLine> for Point {
    type Output = Plane;
    fn bitand(self, l: IdealLine) -> Plane {
        !(!self ^ !l)
    }
}
