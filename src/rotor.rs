...
use crate::util::{
    add_ss, dp_bc, f32x4_abs, f32x4_xor, flip_signs, hi_dp_bc, rcp_nr1, shuffle_scalar,
};
use crate::{Branch, Direction, Line, Motor, Plane, Point, Translator};
...
impl Rotor {
    ...
    pub fn new(ang_rad: f32, x: f32, y: f32, z: f32) -> Rotor {
        ...
        let p1 = f32x4::from_array([half.cos(), x, y, z])
            * f32x4::from_array([1.0, scale, scale, scale]);
        Rotor { p1 }
    }
...
impl Fn<(Plane,)> for Rotor {
    extern "rust-call" fn call(&self, args: (Plane,)) -> Plane {
        Plane {
            p0: sw012::<false, false>(args.0.p0, self.p1),
        }
    }
}
...
impl Fn<(Line,)> for Rotor {
    extern "rust-call" fn call(&self, args: (Line,)) -> Line {
        ...
        Line {
            p1,
            p2: f32x4::from_array([0.0, 0.0, 0.0, 0.0]),
        }
    }
}
...
impl Mul<Motor> for Rotor {
    ...
    fn mul(self, m: Motor) -> Self::Output {
        Motor {
            p1: gp11(m.p1, self.p1),
            p2: gp12::<false>(self.p1, m.p2),
        }
    }
}
