...
use crate::{IdealLine, Line, Motor, Plane, Point, Rotor};
use core_simd::{f32x4, mask32x4};
use std::ops::{Add, AddAssign, Div, DivAssign, Fn, Mul, MulAssign, Neg, Not, Sub, SubAssign};
...
impl FnMut<(Plane,)> for Translator {
    extern "rust-call" fn call_mut(&mut self, args: (Plane,)) -> Plane {
        self.call(args)
    }
}
impl FnOnce<(Plane,)> for Translator {
    type Output = Plane;
...
impl Fn<(Plane,)> for Translator {
    extern "rust-call" fn call(&self, args: (Plane,)) -> Plane {
        ...
        Plane {
            p0: sw02(args.0.p0, tmp),
        }
    }
}

impl FnMut<(Line,)> for Translator {
    extern "rust-call" fn call_mut(&mut self, args: (Line,)) -> Line {
        self.call(args)
    }
}
impl FnOnce<(Line,)> for Translator {
    type Output = Line;
...
impl FnMut<(Point,)> for Translator {
    extern "rust-call" fn call_mut(&mut self, args: (Point,)) -> Point {
        self.call(args)
    }
}
impl FnOnce<(Point,)> for Translator {
    type Output = Point;
