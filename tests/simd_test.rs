...
mod tests {
    use core::arch::x86_64::{
        __m128, _mm_and_ps, _mm_extract_ps, _mm_movehdup_ps, _mm_movehl_ps, _mm_movelh_ps,
        _mm_mul_ps, _mm_set_ps, _mm_set_ss, _mm_sub_ps, _mm_unpacklo_ps, _mm_xor_ps,
    };
    use core_simd::{f32x4, mask32x4};
    use g3::util::{f32x4_abs, f32x4_and, f32x4_andnot, f32x4_xor, flip_signs};
    ...
    #[test]
    fn test_f32_sign_flipping() {
        let v1 = f32x4::from_array([1.0, 2.0, 3.0, 4.0]);
        let expected = f32x4::from_array([-1.0, -2.0, -3.0, 4.0]);
        assert_eq!(
            f32x4_xor(v1, f32x4::from_array([-0.0, -0.0, -0.0, 0.0])),
            expected
        );
        assert_eq!(
            flip_signs(v1, mask32x4::from_array([true, true, true, false])),
            expected
        );
    }
    ...
    fn test_and() {
        ...
        printv(f32x4_and(
            f32x4::from_array([-1.0, 2.0, 3.0, 4.0]),
            f32x4::from_array([0.0, 0.0, 0.0, 0.0]),
        ));
        printv(f32x4_and(
            f32x4::from_array([-1.0, 2.0, 3.0, 4.0]),
            f32x4::from_array([1.0, 0.0, 0.0, 0.0]),
    ...
    fn test_f32x4_abs() {
        assert_eq!(
            f32x4_abs(f32x4::from_array([1.0, -1.0, 0.0, -0.0])),
    ...
    fn test_approx_eq() {
        ...
        println!(
            "and_not_diff: {:?}",
            f32x4_andnot(f32x4::splat(-0.0), a - b)
        );
